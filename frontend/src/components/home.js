...
function Home(props) {
  ...
  //C2- get cam redir
  const routeChange = () => {
    history("/Stream", { state: { apiResult } });
  };
  ...
  if (!username) {
    return <div className="loadingData">loading configuration...</div>;
  } else {
    return (
      <div>
        ...
        <div className="enCamera">
          <div>
            ...
            <p>
              This is a simple contribution web application to Amazon
              Interactive Video Service, for more details please visit the
              project{" "}
              <a href="https://github.com/aws-samples/aws-simple-streaming-webapp">
                GitHub
              </a>
            </p>
          </div>
          {configured | apiResult && (
            <button className="buttonEncam" type="submit" onClick={gotoCam}>
              Enable Cam!
            </button>
          )}
        </div>
        {!configured && (
          <div>
            <p className="configFirst">
              Please configure the IVS paramerters before proceeding:
            </p>
          </div>
        )}
        {errorMSG && (
        ...
        {saved && (
          <div className="saved">
            Channel has been saved for user {username}
          </div>
        )}
